...
Interviewer calculates the label to be used when rendering a node using the the following logic:
  ...
  1. Look for a variable called "name" (regardless of case) in the [codebook](../key-concepts/codebook.md) for node's type, and try to retrieve this value from the node.
