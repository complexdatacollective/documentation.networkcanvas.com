...
## Roster Based Name Generators

The "Small Roster Name Generator" and the "Large Roster Name Generator" Interfaces share some common visual elements.

[image]

The **prompt** area, where the task is explained to the participant (see: [prompts](../key-concepts/prompts.md)), **cards** that display roster alter data to a participant, and a **primary action button** that triggers the node elicitation process similar to the name generators described above.

Depending on the size of your roster, alters will either be created by using a search box to locate and nominate a node or by nominating a node(s) from the list select view.

### Small Roster Name Generator

When using the "Small Roster Name Generator" Interface, cards representing node data from a predetermined roster will be displayed to a participant. By default, cards will display the **name** attribute from the roster data as the card titles and will sort the cards in the order the nodes are defined in the data file. This Interface is intended to accommodate roster data files containing up to 100 nodes. 
...
{: .bad-practices}
- Avoid using this Interface with larger roster datasets that may benefit from the searchability function only available in the Large Roster Name Generator Interface

### Large Roster Name Generator

When using the "Large Roster Name Generator" Interface, the primary action button will trigger a search box to open where a participant can type to search for nodes on a predetermined roster. As the participant begins typing a name, all node data that meets the criteria defined for a match will be displayed as cards above the search box. By default, the cards will display the **name** attribute from the roster data as the card title. This Interface is intended to accommodate roster datasets in excess of 100 nodes.
